...
import com.mvpiq.service.RankingCalculationService;
    ...
    public Response getRankingByRole(@PathParam("role") String roleStr) {
        if (roleStr == null || roleStr.isBlank()) {
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity(ApiResponse.error("Role is required"))
                    .build();
        }
        ...
        if (!ALLOWED_POSITION_CODES.contains(roleCode)) {
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity(ApiResponse.error("Invalid role. Allowed values: " + ALLOWED_POSITION_CODES))
                    .build();
        }
